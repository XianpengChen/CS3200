...
<div class="w3-content w3-padding" style="max-width:1564px">
    ...
    <div class="w3-container w3-padding-32">
        <h1 class="w3-border-bottom w3-border-light-grey w3-padding-16">All registered people are here!</h1>
    </div>
    <div class="w3-container w3-row-padding w3-grayscale">
        <% List<People> ps = (List<People>) request.getAttribute("registered");
            for(People p : ps){%>
    ...
    <div class="w3-container w3-padding-32">
        <h1 class="w3-border-bottom w3-border-light-grey w3-padding-16">Register a people to this Event</h1>
        <form action="${pageContext.request.contextPath}/addPeopleToEvent" method="post">
            <select class="w3-input w3-section w3-border" required name="newRegistration">
                <% List<People> ms = (List<People>) request.getAttribute("notRegistered");
                    for(People p : ms){%>
                <option value="<%= p.getPeopleID()%>"> <%= p.getFirstName() + " " + p.getLastName()%></option>
                <%}%>
            </select>
            <select class="w3-input w3-section w3-border" required name="currentEvent">
    ...
    <div class="w3-container w3-padding-32">
        <h1 class="w3-border-bottom w3-border-light-grey w3-padding-16">Update attendance for a registered people</h1>
        <form action="${pageContext.request.contextPath}/updateEventAttendance" method="post">
            <select class="w3-input w3-section w3-border" required name="currentPeople">
                <% List<People> me = (List<People>) request.getAttribute("registered");
                    for(People p : me){%>
                <option value="<%= p.getPeopleID()%>"> <%= p.getFirstName() + " " + p.getLastName()%></option>
                <%}%>
            </select>
            <select class="w3-input w3-section w3-border" required name="attendance">
            ...
            <input class="w3-button w3-black w3-section" type="submit" value="Update">
        </form>
    </div>
