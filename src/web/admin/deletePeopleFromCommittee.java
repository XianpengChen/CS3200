...
public class deletePeopleFromCommittee extends HttpServlet {
    private DBUtils db;
    public void init() {
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String people_id = request.getParameter("delMember");

        committeePeopleRelation CPR = new committeePeopleRelation(Integer.parseInt(committee_id),
                Integer.parseInt(people_id), "notMatter");
        int key= -1;
        try {
            key = db.modify(CPR.deleteCommitteePeopleRelation());
